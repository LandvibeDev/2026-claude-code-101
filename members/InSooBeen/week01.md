...
그래서 실제 애플리케이션인 orca로 정했다. 병렬 에이전트 오케스트레이션이라는 도메인이 나한테
낯설어서, 어떤 프로젝트인지를 파악해보려는 목적에 적합했음.
...
`festra-perf`는 Festra 성과 측정용 개인 레포다. 코드는 k6 스크립트 2개뿐이지만
`성과측정_설계.md` 설계 문서가 있고, **앞으로 계속 쓸 내 레포**라 CLAUDE.md가 실제로 남아서 쓸모가 있다.
...
파일 구조 요약이 아니라 프로젝트의 **의도**를 문서에서 읽어낸 결과임.
...
> 실제로는 git에 추적 중이고 `.gitignore`에도 없다.
...
전부 틀린 건 아니다. "제품 코드는 여기 없다, 빌드·테스트 대상이 없다"는 정확했고,
k6는 **"별도 설치 필요, 현재 환경에 없음"**이라고 직접 확인해서 명시함.
**실행 파일이 있는지는 확인했는데 디렉터리가 비었는지는 확인하지 않은** 셈.
...
- [`9212cf9` — /init으로 CLAUDE.md 생성](https://github.com/InSooBeen/festra-perf/commit/9212cf9) (원본 그대로)
- [`982e46c` — CLAUDE.md에 현재 상태 명시](https://github.com/InSooBeen/festra-perf/commit/982e46cc91a6994659006252401c7a26b271164c) ← **수정 diff**
